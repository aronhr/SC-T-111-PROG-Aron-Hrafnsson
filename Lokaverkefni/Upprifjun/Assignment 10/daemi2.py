...
"Words" are entities that are seperated by either commas (',') or spaces (' '). In case the string contains
neither commas nor spaces, a list should be returned containing a single element.

Note: We are not testing for the case where both commas and spaces are present in the string so
...
def to_list(string):
    the_string_new = string.split(" ")
    return the_string_new
